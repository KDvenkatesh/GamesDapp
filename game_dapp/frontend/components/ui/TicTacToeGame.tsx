...
export default function TicTacToeGame() {
  ...
  const handlePlayerMove = (index: number) => {
    if (isComputerTurn || !makeMove(index, playerSymbol)) return;
    setXIsNext(false);
    ...
    if (isComputerTurn && !winner && !isDraw) {
      const timer = setTimeout(() => {
        ...
        if (availableMoves.length > 0) {
          const computerMove = availableMoves[Math.floor(Math.random() * availableMoves.length)];
  ...
  }, [isComputerTurn, board, winner, isDraw]);
            ...
            >
              <AnimatePresence>
                {square && (
                  ...
                  >
                    {square === 'X' ? <X className="w-12 h-12 text-cyan-300" /> : <Circle className="w-12 h-12 text-amber-300" />}
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
