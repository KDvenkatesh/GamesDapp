	...
	useEffect(() => {
		...
		return (
			<div className="w-full h-full flex flex-col items-center justify-center bg-black text-white p-8">
				<h2 className="text-2xl font-bold mb-4">Battle Royale Survival</h2>
				...
				<div className="bg-gray-900/60 rounded p-4 mt-4 w-full max-w-md flex flex-col items-center">
					{!started ? (
						<button className="bg-purple-400 text-black px-4 py-2 rounded font-bold" onClick={() => setStarted(true)}>
							Play
						</button>
					) : (
						<div className="font-semibold mb-2">Game Started! Survive the battle!</div>
					)}
				</div>
			</div>
		);
};
export default BattleRoyaleSurvival;
